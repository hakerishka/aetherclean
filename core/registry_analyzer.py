"""
Registry and Installed Software Analyzer for AetherClean.
...
class RegistryAnalyzer:
    ...
    def _load_registry_apps(self):
        ...
        uninstall_paths = [
            (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", getattr(winreg, 'KEY_WOW64_64KEY', 0), "HKLM_64"),
            (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", getattr(winreg, 'KEY_WOW64_32KEY', 0), "HKLM_32"),
            (winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall", 0, "HKCU"),
        ]

        for root_key, sub_key, access_flag, source_label in uninstall_paths:
            try:
    ...
    def _load_msi_registered_packages(self):
        ...
        access_flag = getattr(winreg, 'KEY_WOW64_64KEY', 0)
        for sub_branch in ["Products", "Patches"]:
            full_branch = f"{base_path}\\{sub_branch}"
            try:
                with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, full_branch, 0, winreg.KEY_READ | access_flag) as root_key:
                    ...
                    for i in range(num_subkeys):
                        try:
                            ...
                            try:
                                with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, sub_path, 0, winreg.KEY_READ | access_flag) as prop_key:
    ...
    def is_folder_orphaned(self, folder_name: str, parent_vendor_name: Optional[str] = None) -> Tuple[bool, str]:
        ...
        if parent_vendor_name:
            ...
            if norm_vendor in self.SYSTEM_WHITELIST_KEYWORDS or any(kw in norm_vendor for kw in self.SYSTEM_WHITELIST_KEYWORDS):
                return False, f"Parent vendor '{parent_vendor_name}' is in whitelist"

        for kw in self.SYSTEM_WHITELIST_KEYWORDS:
            if kw in norm_folder or norm_folder in kw:
                return False, f"Matches system component '{kw}'"

        for known_app in self._known_app_names:
            if norm_folder == known_app or (len(norm_folder) > 4 and norm_folder in known_app):
                return False, f"Found installed application: '{known_app}'"

        matching_tokens = [t for t in tokens if t in self._known_tokens and len(t) >= 4]
        if matching_tokens:
            return False, f"Matches installed software token: {', '.join(matching_tokens)}"

        return True, f"No registered application or service found for '{folder_name}'"
